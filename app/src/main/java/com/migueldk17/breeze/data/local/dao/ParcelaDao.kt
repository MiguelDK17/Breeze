...
interface ParcelaDao {
    ...
    @Query("SELECT * FROM parcela WHERE data_vencimento LIKE :mesAno || '%'")
     fun getParcelasDoMes(mesAno: String): Flow<List<ParcelaEntity>>

    @Query("SELECT * FROM parcela WHERE id_conta_pai = :idContaPai AND data_vencimento LIKE :mesAno LIMIT 1")
    fun getParcelaDoMes(idContaPai: Long, mesAno: String): Flow<ParcelaEntity?>
    //Busca todas as parcelas de uma conta específica
    @Query("SELECT * FROM parcela WHERE id_conta_pai = :idContaPai ORDER BY numero_parcela ASC")
    fun getParcelasDaConta(idContaPai: Long): Flow<List<ParcelaEntity>>
    @Query("SELECT * FROM parcela WHERE id = :idParcela")
    ...
    @Query("UPDATE parcela SET esta_paga = 1,  data_pagamento = :data, forma_pagamento = :formaPagamento WHERE id_conta_pai = :idContaPai AND id = :idParcela")
